<nav class="bn">
    ...
    <a href="{Text::url('plugin/postpaid_page')}" {if $_system_menu == 'package'}class="active"{/if}><i class="bi bi-arrow-repeat"></i><span>Package</span></a>
    ...
    <a href="{Text::url('order/history')}" {if ($_system_menu == 'order' && $_routes[1] != 'package') || $_system_menu == 'history'}class="active"{/if}><i class="bi bi-clock-history"></i><span>{Lang::T('History')}</span></a>
